...
const CURATED: Record<MonthKey, Array<{ slug: string; en: string; fr: string; es: string; de: string }>> = {
  january: [
...
export default async function Page({ params }: { params: Promise<{ locale: string; month: string }> }) {
  ...
  const picks = CURATED[m].map((e) => {
    ...
    const reason = e[locale as 'en' | 'fr' | 'es' | 'de'] ?? e.en
