...
const STICKY_LABELS_TRAIN: Record<string, { label: string; cta: string }> = {
  ...
  nl: { label: `Dierenvriendelijke hotels bij grote treinstations`, cta: 'Bekijk hotels' },
  it: { label: `Hotel pet-friendly vicino alle principali stazioni ferroviarie`, cta: 'Vedi hotel' },
}
...
export async function generateMetadata({
  ...
  const titles: Record<string, string> = {
    ...
    nl: 'Met de hond op de trein door Europa: de complete gids 2025',
    it: 'Viaggiare in treno con il cane in Europa: guida completa 2025',
  }
  const descriptions: Record<string, string> = {
    ...
    nl: 'Alles wat je moet weten over treinreizen met je hond in Europa. SNCF, Deutsche Bahn, Renfe, Trenitalia, Eurostar en meer: prijzen, regels, muilkorfplicht en praktische tips.',
    it: 'Tutto quello che devi sapere per viaggiare in treno con il cane in Europa. SNCF, Deutsche Bahn, Renfe, Trenitalia, Eurostar e altro: prezzi, regole, obbligo di museruola e consigli pratici.',
  }
...
const OPERATORS: Operator[] = [
  {
    ...
    flag: '🇫🇷',
    name: 'SNCF / TGV INOUI',
    country: { fr: 'France', en: 'France', es: 'Francia', pt: 'Francia' , de: 'Frankreich', nl: 'Frankrijk', it: 'Francia' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui', en: 'Yes', es: 'Sí', pt: 'Sí' , de: 'Ja', nl: 'Ja', it: 'Sì' },
    price: { fr: '10 € (panier) · 20 € (en laisse)', en: '€10 (carrier) · €20 (on lead)', es: '10 € (transportín) · 20 € (con correa)', pt: '10 € (transportadora) · 20 € (com trela)' , de: '10 € (Transportbox) · 20 € (an der Leine)', nl: '10 € (transportmand) · 20 € (aan de lijn)', it: '10 € (trasportino) · 20 € (al guinzaglio)' },
    sizeLimit: { fr: 'Panier < 6 kg : 45×30×25 cm. Grand chien en laisse.', en: 'Carrier < 6 kg: 45×30×25 cm. Large dog on lead.', es: 'Transportín < 6 kg: 45×30×25 cm. Perro grande con correa.', pt: 'Transportadora < 6 kg: 45×30×25 cm. Cão grande com trela.' , de: 'Transportbox < 6 kg: 45×30×25 cm. Großer Hund an der Leine.', nl: 'Transportmand < 6 kg: 45×30×25 cm. Grote hond aan de lijn.', it: 'Trasportino < 6 kg: 45×30×25 cm. Cane grande al guinzaglio.' },
    muzzle: { fr: 'Obligatoire > 6 kg', en: 'Mandatory > 6 kg', es: 'Obligatorio > 6 kg', pt: 'Obrigatório > 6 kg' , de: 'Pflicht > 6 kg', nl: 'Verplicht > 6 kg', it: 'Obbligatoria > 6 kg' },
    carrier: { fr: 'Obligatoire < 6 kg', en: 'Mandatory < 6 kg', es: 'Obligatorio < 6 kg', pt: 'Obrigatório < 6 kg' , de: 'Pflicht < 6 kg', nl: 'Verplicht < 6 kg', it: 'Obbligatorio < 6 kg' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Chiens de cat. 1 interdits. Chiens guides gratuits.', en: 'Category 1 dogs banned. Guide dogs free.', es: 'Perros de cat. 1 prohibidos. Perros guía gratis.', pt: 'Cães de cat. 1 prohibidos. Cães guía grátis.' , de: 'Hunde der Kategorie 1 verboten. Blindenführhunde kostenlos.', nl: 'Honden van categorie 1 verboden. Blindengeleidehonden gratis.', it: 'Cani di cat. 1 vietati. Cani guida gratis.' },
  },
  {
    slug: 'ouigo',
    flag: '🇫🇷',
    ...
    allowed: 'yes',
    ticketRequired: { fr: 'Oui', en: 'Yes', es: 'Sí', pt: 'Sí' , de: 'Ja', nl: 'Ja', it: 'Sì' },
    price: { fr: '10 € (tous poids)', en: '€10 (all sizes)', es: '10 € (todos los tamaños)', pt: '10 € (todos os tamanhos)' , de: '10 € (alle Größen)', nl: '10 € (alle formaten)', it: '10 € (tutte le taglie)' },
    sizeLimit: { fr: 'Aucune limite de poids', en: 'No weight limit', es: 'Sin límite de peso', pt: 'Sem limite de peso' , de: 'Kein Gewichtslimit', nl: 'Geen gewichtslimiet', it: 'Nessun limite di peso' },
    muzzle: { fr: 'Obligatoire', en: 'Mandatory', es: 'Obligatorio', pt: 'Obrigatório' , de: 'Pflicht', nl: 'Verplicht', it: 'Obbligatoria' },
    carrier: { fr: 'Non requis (laisse suffisante)', en: 'Not required (lead sufficient)', es: 'No requerido (correa suficiente)', pt: 'No requerido (trela suficiente)' , de: 'Nicht erforderlich (Leine ausreichend)', nl: 'Niet verplicht (lijn is voldoende)', it: 'Non richiesto (basta il guinzaglio)' },
    maxDogs: { fr: '1 par passager adulte', en: '1 per adult passenger', es: '1 por pasajero adulto', pt: '1 por pasajero adulto' , de: '1 pro erwachsenem Fahrgast', nl: '1 per volwassen reiziger', it: '1 per passeggero adulto' },
    notes: { fr: 'Carnet de vaccination obligatoire. Cat. 1 interdits.', en: 'Vaccination booklet mandatory. Cat. 1 banned.', es: 'Cartilla de vacunación obligatoria. Cat. 1 prohibidos.', pt: 'Cartilla de vacinação obrigatória. Cat. 1 prohibidos.' , de: 'Impfausweis Pflicht. Kategorie 1 verboten.', nl: 'Vaccinatieboekje verplicht. Categorie 1 verboden.', it: 'Libretto di vaccinazione obbligatorio. Cat. 1 vietati.' },
  },
  {
    slug: 'eurostar',
    flag: '🇬🇧',
    name: 'Eurostar',
    country: { fr: 'UK–France–Belgique', en: 'UK–France–Belgium', es: 'RU–Francia–Bélgica', pt: 'RU–Francia–Bélgica' , de: 'UK–Frankreich–Belgien', nl: 'VK–Frankrijk–België', it: 'UK–Francia–Belgio' },
    allowed: 'partial',
    ticketRequired: { fr: 'Oui (routes continentales)', en: 'Yes (continental routes)', es: 'Sí (rutas continentales)', pt: 'Sí (rutas continentales)' , de: 'Ja (Festlandstrecken)', nl: 'Ja (routes op het vasteland)', it: 'Sì (tratte continentali)' },
    price: { fr: 'Gratuit < 6 kg · 30 € > 6 kg (routes continentales)', en: 'Free < 6 kg · €30 > 6 kg (continental routes)', es: 'Gratis < 6 kg · 30 € > 6 kg (rutas continentales)', pt: 'Grátis < 6 kg · 30 € > 6 kg (rutas continentales)' , de: 'Kostenlos < 6 kg · 30 € > 6 kg (Festlandstrecken)', nl: 'Gratis < 6 kg · 30 € > 6 kg (routes op het vasteland)', it: 'Gratis < 6 kg · 30 € > 6 kg (tratte continentali)' },
    sizeLimit: { fr: '< 6 kg en panier 45×30×25 cm. Grand chien en laisse.', en: '< 6 kg in 45×30×25 cm carrier. Large dog on lead.', es: '< 6 kg en transportín 45×30×25 cm. Perro grande con correa.', pt: '< 6 kg en transportadora 45×30×25 cm. Cão grande com trela.' , de: '< 6 kg in Transportbox 45×30×25 cm. Großer Hund an der Leine.', nl: '< 6 kg in transportmand 45×30×25 cm. Grote hond aan de lijn.', it: '< 6 kg in trasportino 45×30×25 cm. Cane grande al guinzaglio.' },
    muzzle: { fr: 'Obligatoire > 6 kg', en: 'Mandatory > 6 kg', es: 'Obligatorio > 6 kg', pt: 'Obrigatório > 6 kg' , de: 'Pflicht > 6 kg', nl: 'Verplicht > 6 kg', it: 'Obbligatoria > 6 kg' },
    carrier: { fr: 'Obligatoire < 6 kg', en: 'Mandatory < 6 kg', es: 'Obligatorio < 6 kg', pt: 'Obrigatório < 6 kg' , de: 'Pflicht < 6 kg', nl: 'Verplicht < 6 kg', it: 'Obbligatorio < 6 kg' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: '⚠️ Animaux INTERDITS sur les trains vers/depuis Londres (tunnel sous la Manche). Autorisés sur routes Belgique–France–Allemagne–Pays-Bas.', en: '⚠️ Pets BANNED on trains to/from London (Channel Tunnel). Allowed on Belgium–France–Germany–Netherlands routes.', es: '⚠️ Animales PROHIBIDOS en trenes hacia/desde Londres (Eurotúnel). Permitidos en rutas Bélgica–Francia–Alemania–Países Bajos.', pt: '⚠️ Animales PROHIBIDOS en comboios hacia/a partir de Londres (Eurotúnel). Permitidos en rutas Bélgica–Francia–Alemania–Países Bajos.' , de: '⚠️ Tiere auf Zügen von/nach London VERBOTEN (Ärmelkanaltunnel). Erlaubt auf den Strecken Belgien–Frankreich–Deutschland–Niederlande.', nl: '⚠️ Huisdieren VERBODEN op treinen van/naar Londen (Kanaaltunnel). Toegestaan op de routes België–Frankrijk–Duitsland–Nederland.', it: '⚠️ Animali VIETATI sui treni da/per Londra (tunnel della Manica). Ammessi sulle tratte Belgio–Francia–Germania–Paesi Bassi.' },
  },
  {
    slug: 'deutsche-bahn',
    flag: '🇩🇪',
    name: 'Deutsche Bahn (DB)',
    country: { fr: 'Allemagne', en: 'Germany', es: 'Alemania', pt: 'Alemania' , de: 'Deutschland', nl: 'Duitsland', it: 'Germania' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (grands chiens)', en: 'Yes (large dogs)', es: 'Sí (perros grandes)', pt: 'Sí (cães grandes)' , de: 'Ja (große Hunde)', nl: 'Ja (grote honden)', it: 'Sì (cani grandi)' },
    price: { fr: 'Gratuit en panier (taille chat). Demi-tarif adulte (grande distance) · plein tarif (régional)', en: 'Free in carrier (cat-sized). Half adult fare (long-distance) · full fare (regional)', es: 'Gratis en transportín (talla gato). Mitad tarifa adulto (larga distancia) · tarifa completa (regional)', pt: 'Grátis en transportadora (talla gato). Mitad tarifa adulto (larga distancia) · tarifa completa (regional)' , de: 'Kostenlos in Transportbox (Katzengröße). Halber Erwachsenenpreis (Fernverkehr) · voller Preis (Nahverkehr)', nl: 'Gratis in transportmand (kattenformaat). Halve volwassenenprijs (langeafstandstreinen) · volle prijs (regionaal)', it: 'Gratis in trasportino (taglia gatto). Metà tariffa adulto (lunga percorrenza) · tariffa intera (regionale)' },
    sizeLimit: { fr: 'Petit animal en panier gratuit. Grand chien : billet obligatoire.', en: 'Small pet in carrier free. Large dog: ticket required.', es: 'Animal pequeño en transportín gratis. Perro grande: billete obligatorio.', pt: 'Animal pequenho en transportadora grátis. Cão grande: bilhete obrigatório.' , de: 'Kleines Tier in Transportbox kostenlos. Großer Hund: Fahrkarte erforderlich.', nl: 'Klein huisdier in transportmand gratis. Grote hond: kaartje verplicht.', it: 'Animale piccolo in trasportino gratis. Cane grande: biglietto obbligatorio.' },
    muzzle: { fr: 'Obligatoire (grands chiens)', en: 'Mandatory (large dogs)', es: 'Obligatorio (perros grandes)', pt: 'Obrigatório (cães grandes)' , de: 'Pflicht (große Hunde)', nl: 'Verplicht (grote honden)', it: 'Obbligatoria (cani grandi)' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Billet chien réservable en ligne (app DB Next Navigator) ou en gare.', en: 'Dog ticket bookable online (DB Next Navigator app) or at station.', es: 'Billete perro reservable online (app DB Next Navigator) o en estación.', pt: 'Bilhete cão reservable online (app DB Next Navigator) o en estação.' , de: 'Hundefahrkarte online buchbar (App DB Next Navigator) oder am Bahnhof.', nl: 'Hondenkaartje online boekbaar (app DB Next Navigator) of op het station.', it: 'Biglietto cane prenotabile online (app DB Next Navigator) o in stazione.' },
  },
  {
    slug: 'renfe',
    flag: '🇪🇸',
    name: 'Renfe',
    country: { fr: 'Espagne', en: 'Spain', es: 'España', pt: 'Espanha' , de: 'Spanien', nl: 'Spanje', it: 'Spagna' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui', en: 'Yes', es: 'Sí', pt: 'Sí' , de: 'Ja', nl: 'Ja', it: 'Sì' },
    price: { fr: '10 € (Avlo) · 35 € sans cage (trains AVE avec sièges chiens) · Gratuit (Media Distancia)', en: '€10 (Avlo) · €35 without crate (AVE with dog seats) · Free (Media Distancia)', es: '10 € (Avlo) · 35 € sin jaula (AVE con asientos perros) · Gratis (Media Distancia)', pt: '10 € (Avlo) · 35 € sem jaula (AVE com asientos cães) · Grátis (Media Distancia)' , de: '10 € (Avlo) · 35 € ohne Box (AVE mit Hundeplätzen) · Kostenlos (Media Distancia)', nl: '10 € (Avlo) · 35 € zonder bench (AVE met hondenplaatsen) · Gratis (Media Distancia)', it: '10 € (Avlo) · 35 € senza gabbia (AVE con posti cani) · Gratis (Media Distancia)' },
    sizeLimit: { fr: 'En cage : 60×35×35 cm max. Sans cage jusqu\'à 40 kg sur trains équipés.', en: 'In crate: max 60×35×35 cm. No crate up to 40 kg on fitted trains.', es: 'En jaula: máx 60×35×35 cm. Sin jaula hasta 40 kg en trenes equipados.', pt: 'En jaula: máx 60×35×35 cm. Sem jaula até 40 kg en comboios equipados.' , de: 'In Box: max. 60×35×35 cm. Ohne Box bis 40 kg in ausgestatteten Zügen.', nl: 'In bench: max. 60×35×35 cm. Zonder bench tot 40 kg in uitgeruste treinen.', it: 'In gabbia: max 60×35×35 cm. Senza gabbia fino a 40 kg sui treni attrezzati.' },
    muzzle: { fr: 'Obligatoire (Media Distancia)', en: 'Mandatory (Media Distancia)', es: 'Obligatorio (Media Distancia)', pt: 'Obrigatório (Media Distancia)' , de: 'Pflicht (Media Distancia)', nl: 'Verplicht (Media Distancia)', it: 'Obbligatoria (Media Distancia)' },
    carrier: { fr: 'Obligatoire (AVE, sauf sièges chiens)', en: 'Mandatory (AVE, except dog seats)', es: 'Obligatorio (AVE, salvo asientos perros)', pt: 'Obrigatório (AVE, salvo asientos cães)' , de: 'Pflicht (AVE, außer Hundeplätze)', nl: 'Verplicht (AVE, behalve hondenplaatsen)', it: 'Obbligatorio (AVE, tranne posti cani)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Présenter carnet de vaccination + assurance RC à l\'enregistrement (40 min avant départ).', en: 'Show vaccination booklet + liability insurance at check-in (40 min before departure).', es: 'Presentar cartilla de vacunación + seguro RC en facturación (40 min antes de salida).', pt: 'Presentar cartilla de vacinação + seguro RC en facturación (40 min antes de salida).' , de: 'Impfausweis + Haftpflichtversicherung beim Check-in vorlegen (40 Min. vor Abfahrt).', nl: 'Vaccinatieboekje + aansprakelijkheidsverzekering tonen bij de check-in (40 min. voor vertrek).', it: 'Presentare libretto di vaccinazione + assicurazione RC al check-in (40 min prima della partenza).' },
  },
  {
    slug: 'trenitalia',
    flag: '🇮🇹',
    name: 'Trenitalia',
    country: { fr: 'Italie', en: 'Italy', es: 'Italia', pt: 'Italia' , de: 'Italien', nl: 'Italië', it: 'Italia' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (grand chien)', en: 'Yes (large dog)', es: 'Sí (perro grande)', pt: 'Sí (cão grande)' , de: 'Ja (großer Hund)', nl: 'Ja (grote hond)', it: 'Sì (cane grande)' },
    price: { fr: 'Gratuit (panier < 70×30×50 cm). 50 % du tarif 2e classe (grand chien).', en: 'Free (carrier < 70×30×50 cm). 50% of 2nd class fare (large dog).', es: 'Gratis (transportín < 70×30×50 cm). 50% tarifa 2ª clase (perro grande).', pt: 'Grátis (transportadora < 70×30×50 cm). 50% tarifa 2ª clase (cão grande).' , de: 'Kostenlos (Box < 70×30×50 cm). 50 % des 2.-Klasse-Preises (großer Hund).', nl: 'Gratis (bench < 70×30×50 cm). 50% van de prijs 2e klasse (grote hond).', it: 'Gratis (trasportino < 70×30×50 cm). 50% della tariffa di 2ª classe (cane grande).' },
    sizeLimit: { fr: 'Petit en panier gratuit. Grand chien en laisse sur Frecce, Intercity.', en: 'Small in carrier free. Large dog on lead on Frecce, Intercity.', es: 'Pequeño en transportín gratis. Perro grande con correa en Frecce, Intercity.', pt: 'Pequenho en transportadora grátis. Cão grande com trela en Frecce, Intercity.' , de: 'Kleiner Hund in Box kostenlos. Großer Hund an der Leine bei Frecce, Intercity.', nl: 'Kleine hond in bench gratis. Grote hond aan de lijn bij Frecce, Intercity.', it: 'Cane piccolo in trasportino gratis. Cane grande al guinzaglio su Frecce, Intercity.' },
    muzzle: { fr: 'Obligatoire (grands chiens)', en: 'Mandatory (large dogs)', es: 'Obligatorio (perros grandes)', pt: 'Obrigatório (cães grandes)' , de: 'Pflicht (große Hunde)', nl: 'Verplicht (grote honden)', it: 'Obbligatoria (cani grandi)' },
    carrier: { fr: 'Obligatoire (petits)', en: 'Mandatory (small)', es: 'Obligatorio (pequeños)', pt: 'Obrigatório (pequenhos)' , de: 'Pflicht (kleine Hunde)', nl: 'Verplicht (kleine honden)', it: 'Obbligatorio (piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Carnet canin + livret sanitaire obligatoires. Promo été 2025 : grands chiens gratuits sur Frecce et Intercity.', en: 'Dog registration + health booklet mandatory. Summer 2025 promo: large dogs free on Frecce and Intercity.', es: 'Registro canino + libreta sanitaria obligatorios. Promo verano 2025: perros grandes gratis en Frecce e Intercity.', pt: 'Registro canino + libreta sanitaria obrigatórios. Promo verão 2025: cães grandes grátis en Frecce e Intercity.' , de: 'Hunderegistrierung + Gesundheitsheft Pflicht. Sommeraktion 2025: große Hunde kostenlos bei Frecce und Intercity.', nl: 'Hondenregistratie + gezondheidsboekje verplicht. Zomeractie 2025: grote honden gratis bij Frecce en Intercity.', it: 'Iscrizione all\'anagrafe canina + libretto sanitario obbligatori. Promo estate 2025: cani grandi gratis su Frecce e Intercity.' },
  },
  {
    slug: 'sncb',
    flag: '🇧🇪',
    name: 'SNCB / NMBS',
    country: { fr: 'Belgique', en: 'Belgium', es: 'Bélgica', pt: 'Bélgica' , de: 'Belgien', nl: 'België', it: 'Belgio' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (en laisse)', en: 'Yes (on lead)', es: 'Sí (con correa)', pt: 'Sí (com trela)' , de: 'Ja (an der Leine)', nl: 'Ja (aan de lijn)', it: 'Sì (al guinzaglio)' },
    price: { fr: 'Gratuit (panier 30×55×30 cm max). Supplément animal en laisse.', en: 'Free (carrier max 30×55×30 cm). Pet supplement on lead.', es: 'Gratis (transportín máx 30×55×30 cm). Suplemento mascota con correa.', pt: 'Grátis (transportadora máx 30×55×30 cm). Suplemento animal com trela.' , de: 'Kostenlos (Box max. 30×55×30 cm). Zuschlag für Tier an der Leine.', nl: 'Gratis (bench max. 30×55×30 cm). Toeslag voor huisdier aan de lijn.', it: 'Gratis (trasportino max 30×55×30 cm). Supplemento per animale al guinzaglio.' },
    sizeLimit: { fr: 'Panier gratuit. Grand chien : supplément requis.', en: 'Carrier free. Large dog: supplement required.', es: 'Transportín gratis. Perro grande: suplemento requerido.', pt: 'Transportadora grátis. Cão grande: suplemento requerido.' , de: 'Box kostenlos. Großer Hund: Zuschlag erforderlich.', nl: 'Bench gratis. Grote hond: toeslag verplicht.', it: 'Trasportino gratis. Cane grande: supplemento richiesto.' },
    muzzle: { fr: 'Non obligatoire (recommandée)', en: 'Not mandatory (recommended)', es: 'No obligatorio (recomendado)', pt: 'No obrigatório (recomendado)' , de: 'Nicht Pflicht (empfohlen)', nl: 'Niet verplicht (aanbevolen)', it: 'Non obbligatoria (consigliata)' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Chiens guides gratuits et sans muselière obligatoire.', en: 'Guide dogs free and no muzzle required.', es: 'Perros guía gratis y sin bozal obligatorio.', pt: 'Cães guía grátis e sem bozal obrigatório.' , de: 'Blindenführhunde kostenlos und ohne Maulkorbpflicht.', nl: 'Blindengeleidehonden gratis en zonder muilkorfplicht.', it: 'Cani guida gratis e senza obbligo di museruola.' },
  },
  {
    slug: 'ns',
    flag: '🇳🇱',
    name: 'NS (Nederlandse Spoorwegen)',
    country: { fr: 'Pays-Bas', en: 'Netherlands', es: 'Países Bajos', pt: 'Países Bajos' , de: 'Niederlande', nl: 'Nederland', it: 'Paesi Bassi' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (grand chien)', en: 'Yes (large dog)', es: 'Sí (perro grande)', pt: 'Sí (cão grande)' , de: 'Ja (großer Hund)', nl: 'Ja (grote hond)', it: 'Sì (cane grande)' },
    price: { fr: 'Gratuit (panier 55×30×30 cm). Dagkaart Hond : 3,50 € (e-ticket) / 5 € (papier).', en: 'Free (carrier 55×30×30 cm). Dagkaart Hond: €3.50 (e-ticket) / €5 (paper).', es: 'Gratis (transportín 55×30×30 cm). Dagkaart Hond: 3,50 € (e-ticket) / 5 € (papel).', pt: 'Grátis (transportadora 55×30×30 cm). Dagkaart Hond: 3,50 € (e-ticket) / 5 € (papel).' , de: 'Kostenlos (Box 55×30×30 cm). Dagkaart Hond: 3,50 € (E-Ticket) / 5 € (Papier).', nl: 'Gratis (bench 55×30×30 cm). Dagkaart Hond: € 3,50 (e-ticket) / € 5 (papier).', it: 'Gratis (trasportino 55×30×30 cm). Dagkaart Hond: 3,50 € (e-ticket) / 5 € (cartaceo).' },
    sizeLimit: { fr: 'Panier < 55×30×30 cm gratuit. Grand chien : laisse + muselière.', en: 'Carrier < 55×30×30 cm free. Large dog: lead + muzzle.', es: 'Transportín < 55×30×30 cm gratis. Perro grande: correa + bozal.', pt: 'Transportadora < 55×30×30 cm grátis. Cão grande: trela + bozal.' , de: 'Box < 55×30×30 cm kostenlos. Großer Hund: Leine + Maulkorb.', nl: 'Bench < 55×30×30 cm gratis. Grote hond: lijn + muilkorf.', it: 'Trasportino < 55×30×30 cm gratis. Cane grande: guinzaglio + museruola.' },
    muzzle: { fr: 'Obligatoire (grands chiens)', en: 'Mandatory (large dogs)', es: 'Obligatorio (perros grandes)', pt: 'Obrigatório (cães grandes)' , de: 'Pflicht (große Hunde)', nl: 'Verplicht (grote honden)', it: 'Obbligatoria (cani grandi)' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Dagkaart Hond valable toute la journée sur tout le réseau domestique NS.', en: 'Dagkaart Hond valid all day on all domestic NS routes.', es: 'Dagkaart Hond válido todo el día en toda la red doméstica NS.', pt: 'Dagkaart Hond válido todo o dia en toda a rede doméstica NS.' , de: 'Dagkaart Hond den ganzen Tag gültig im gesamten NS-Inlandsnetz.', nl: 'Dagkaart Hond de hele dag geldig op het volledige binnenlandse NS-netwerk.', it: 'Dagkaart Hond valida tutto il giorno su tutta la rete nazionale NS.' },
  },
  {
    slug: 'sbb',
    flag: '🇨🇭',
    name: 'SBB / CFF / FFS',
    country: { fr: 'Suisse', en: 'Switzerland', es: 'Suiza', pt: 'Suiza' , de: 'Schweiz', nl: 'Zwitserland', it: 'Svizzera' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (grand chien)', en: 'Yes (large dog)', es: 'Sí (perro grande)', pt: 'Sí (cão grande)' , de: 'Ja (großer Hund)', nl: 'Ja (grote hond)', it: 'Sì (cane grande)' },
    price: { fr: 'Gratuit (en sac/panier). Abonnement journalier chien : CHF 25.', en: 'Free (in bag/carrier). Dog day pass: CHF 25.', es: 'Gratis (en bolsa/transportín). Abono diario perro: CHF 25.', pt: 'Grátis (en bolsa/transportadora). Abono diario cão: CHF 25.' , de: 'Kostenlos (in Tasche/Box). Hunde-Tageskarte: CHF 25.', nl: 'Gratis (in tas/bench). Hondendagkaart: CHF 25.', it: 'Gratis (in borsa/trasportino). Abbonamento giornaliero cane: CHF 25.' },
    sizeLimit: { fr: 'Petit en sac/panier gratuit. Grand chien : abonnement jour.', en: 'Small in bag/carrier free. Large dog: day pass.', es: 'Pequeño en bolsa/transportín gratis. Perro grande: abono día.', pt: 'Pequenho en bolsa/transportadora grátis. Cão grande: abono dia.' , de: 'Kleiner Hund in Tasche/Box kostenlos. Großer Hund: Tageskarte.', nl: 'Kleine hond in tas/bench gratis. Grote hond: dagkaart.', it: 'Cane piccolo in borsa/trasportino gratis. Cane grande: abbonamento giornaliero.' },
    muzzle: { fr: 'Recommandée', en: 'Recommended', es: 'Recomendado', pt: 'Recomendado' , de: 'Empfohlen', nl: 'Aanbevolen', it: 'Consigliata' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Le TGV Lyria (France–Suisse) suit les tarifs SNCF : 10 € (panier) ou 20 € (laisse).', en: 'TGV Lyria (France–Switzerland) follows SNCF rates: €10 (carrier) or €20 (lead).', es: 'TGV Lyria (Francia–Suiza) sigue tarifas SNCF: 10 € (transportín) o 20 € (correa).', pt: 'TGV Lyria (Francia–Suiza) sigue tarifas SNCF: 10 € (transportadora) o 20 € (trela).' , de: 'Der TGV Lyria (Frankreich–Schweiz) folgt den SNCF-Tarifen: 10 € (Box) oder 20 € (Leine).', nl: 'De TGV Lyria (Frankrijk–Zwitserland) volgt de SNCF-tarieven: 10 € (bench) of 20 € (lijn).', it: 'Il TGV Lyria (Francia–Svizzera) segue le tariffe SNCF: 10 € (trasportino) o 20 € (guinzaglio).' },
  },
  {
    slug: 'obb',
    flag: '🇦🇹',
    name: 'ÖBB',
    country: { fr: 'Autriche', en: 'Austria', es: 'Austria', pt: 'Austria' , de: 'Österreich', nl: 'Oostenrijk', it: 'Austria' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui (grand chien)', en: 'Yes (large dog)', es: 'Sí (perro grande)', pt: 'Sí (cão grande)' , de: 'Ja (großer Hund)', nl: 'Ja (grote hond)', it: 'Sì (cane grande)' },
    price: { fr: 'Gratuit (panier 70×30×50 cm). Grand chien : billet séparé + compartiment privatif.', en: 'Free (carrier 70×30×50 cm). Large dog: separate ticket + private compartment.', es: 'Gratis (transportín 70×30×50 cm). Perro grande: billete separado + compartimento privado.', pt: 'Grátis (transportadora 70×30×50 cm). Cão grande: bilhete separado + compartimento privado.' , de: 'Kostenlos (Box 70×30×50 cm). Großer Hund: separate Fahrkarte + eigenes Abteil.', nl: 'Gratis (bench 70×30×50 cm). Grote hond: apart kaartje + eigen coupé.', it: 'Gratis (trasportino 70×30×50 cm). Cane grande: biglietto separato + scompartimento privato.' },
    sizeLimit: { fr: 'Panier 70×30×50 cm gratuit. Grand chien : compartiment entier requis.', en: 'Carrier 70×30×50 cm free. Large dog: full compartment required.', es: 'Transportín 70×30×50 cm gratis. Perro grande: compartimento completo requerido.', pt: 'Transportadora 70×30×50 cm grátis. Cão grande: compartimento completo requerido.' , de: 'Box 70×30×50 cm kostenlos. Großer Hund: ganzes Abteil erforderlich.', nl: 'Bench 70×30×50 cm gratis. Grote hond: hele coupé verplicht.', it: 'Trasportino 70×30×50 cm gratis. Cane grande: scompartimento intero richiesto.' },
    muzzle: { fr: 'Obligatoire (grands chiens)', en: 'Mandatory (large dogs)', es: 'Obligatorio (perros grandes)', pt: 'Obrigatório (cães grandes)' , de: 'Pflicht (große Hunde)', nl: 'Verplicht (grote honden)', it: 'Obbligatoria (cani grandi)' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Laisse et muselière obligatoires pour grands chiens.', en: 'Lead and muzzle mandatory for large dogs.', es: 'Correa y bozal obligatorios para perros grandes.', pt: 'Trela e bozal obrigatórios para cães grandes.' , de: 'Leine und Maulkorb für große Hunde Pflicht.', nl: 'Lijn en muilkorf verplicht voor grote honden.', it: 'Guinzaglio e museruola obbligatori per cani grandi.' },
  },
  {
    slug: 'pkp',
    flag: '🇵🇱',
    name: 'PKP Intercity',
    country: { fr: 'Pologne', en: 'Poland', es: 'Polonia', pt: 'Polonia' , de: 'Polen', nl: 'Polen', it: 'Polonia' },
    allowed: 'yes',
    ticketRequired: { fr: 'Oui', en: 'Yes', es: 'Sí', pt: 'Sí' , de: 'Ja', nl: 'Ja', it: 'Sì' },
    price: { fr: 'Billet animal requis (tarif variable)', en: 'Pet ticket required (variable fare)', es: 'Billete mascota requerido (tarifa variable)', pt: 'Bilhete animal requerido (tarifa variable)' , de: 'Tierfahrkarte erforderlich (variabler Preis)', nl: 'Dierenkaartje verplicht (variabele prijs)', it: 'Biglietto animale richiesto (tariffa variabile)' },
    sizeLimit: { fr: 'Petit en panier accepté. Grand chien en laisse.', en: 'Small in carrier accepted. Large dog on lead.', es: 'Pequeño en transportín aceptado. Perro grande con correa.', pt: 'Pequenho en transportadora aceite. Cão grande com trela.' , de: 'Kleiner Hund in Box akzeptiert. Großer Hund an der Leine.', nl: 'Kleine hond in bench toegestaan. Grote hond aan de lijn.', it: 'Cane piccolo in trasportino accettato. Cane grande al guinzaglio.' },
    muzzle: { fr: 'Obligatoire', en: 'Mandatory', es: 'Obligatorio', pt: 'Obrigatório' , de: 'Pflicht', nl: 'Verplicht', it: 'Obbligatoria' },
    carrier: { fr: 'Obligatoire (petits animaux)', en: 'Mandatory (small pets)', es: 'Obligatorio (animales pequeños)', pt: 'Obrigatório (animales pequenhos)' , de: 'Pflicht (kleine Tiere)', nl: 'Verplicht (kleine huisdieren)', it: 'Obbligatorio (animali piccoli)' },
    maxDogs: { fr: '1 par passager', en: '1 per passenger', es: '1 por pasajero', pt: '1 por pasajero' , de: '1 pro Fahrgast', nl: '1 per reiziger', it: '1 per passeggero' },
    notes: { fr: 'Chiens interdits dans les voitures-lits et couchettes EuroNight PKP.', en: 'Dogs banned in EuroNight PKP couchette and sleeping cars.', es: 'Perros prohibidos en coches cama y literas EuroNight PKP.', pt: 'Cães prohibidos en carros cama e literas EuroNight PKP.' , de: 'Hunde in EuroNight-PKP-Liege- und Schlafwagen verboten.', nl: 'Honden verboden in EuroNight PKP-lig- en slaapwagens.', it: 'Cani vietati nelle carrozze letto e cuccette EuroNight PKP.' },
  },
]
...
const COPY = {
  ...
  nl: {
    hero: 'Met de hond op de trein door Europa',
    subtitle: 'SNCF, Deutsche Bahn, Renfe, Trenitalia, Eurostar... De complete gids met regels, prijzen, muilkorfplicht en praktische tips voor een ontspannen treinreis met je hond.',
    lastUpdate: 'Bijgewerkt in',
    breadcrumbGuides: 'Gidsen',
    breadcrumbCurrent: 'Trein met hond',
    badge: 'Praktische gids',

    keyFactsTitle: 'De belangrijkste regels in één oogopslag',
    keyFacts: [
      { icon: '🎫', title: 'Kaartje verplicht', text: 'Overal in Europa heeft een grote hond een kaartje nodig. Kleine huisdieren in een transportmand reizen vaak gratis.' },
      { icon: '😷', title: 'Muilkorf verplicht', text: 'In Frankrijk (SNCF, Ouigo), Italië, Oostenrijk en de meeste landen is een muilkorf verplicht voor honden boven 6 kg.' },
      { icon: '🚫', title: 'Eurostar tunnel = verboden', text: 'Huisdieren (behalve blindengeleidehonden) zijn strikt verboden op Eurostar-treinen door de Kanaaltunnel tussen Londen en het Europese vasteland.' },
      { icon: '📋', title: 'Gezondheidsboekje', text: 'Neem altijd het vaccinatieboekje van je hond mee. Renfe (Spanje) vraagt ook om een aansprakelijkheidsverzekering.' },
      { icon: '📏', title: 'Afmetingen transportmand', text: 'Het standaardformaat voor kleine huisdieren is 45×30×25 cm (SNCF). Sommige maatschappijen accepteren tot 70×30×50 cm.' },
      { icon: '🐕‍🦺', title: 'Blindengeleidehonden gratis', text: 'Assistentie- en blindengeleidehonden reizen gratis door heel Europa, zonder muilkorfplicht.' },
    ],

    tableTitle: 'Vergelijking van Europese spoorwegmaatschappijen',
    colOperator: 'Maatschappij',
    colAllowed: 'Toegestaan?',
    colTicket: 'Kaartje?',
    colPrice: 'Prijs',
    colSize: 'Formaat',
    colMuzzle: 'Muilkorf',
    colCarrier: 'Transportmand',
    colMax: 'Max. / reiziger',
    colNotes: 'Opmerkingen',

    allowedYes: 'Ja',
    allowedPartial: 'Deels',
    allowedNo: 'Nee',

    sncfTitle: 'SNCF en Ouigo: met je hond door Frankrijk',
    sncfContent: [
      'SNCF is de belangrijkste maatschappij voor onze gebruikers in Frankrijk. De regels zijn duidelijk en tamelijk soepel.',
      'Bij de TGV INOUI en Intercités heb je twee opties: (1) je hond weegt minder dan 6 kg en reist in een gesloten transportmand (max. 45×30×25 cm) voor 10 €; (2) je hond is groter en reist aan de lijn, met muilkorf, aan je voeten voor 20 €.',
      'Bij OUIGO, het budgetlabel van SNCF, is het nog eenvoudiger: één vaste prijs van 10 € voor alle honden, ongeacht hun formaat. De muilkorf blijft verplicht.',
      'Honden van categorie 1 (pitbull-type, mastiff, tosa) zijn strikt verboden in alle treinen. Honden van categorie 2 mogen wel mee, maar moeten een muilkorf dragen en aan de lijn blijven.',
      'Boek het kaartje van je hond tegelijk met je eigen kaartje via SNCF Connect of de OUIGO-app. Maximaal één dier per volwassen reiziger.',
      'Op de TGV Lyria (Frankrijk–Zwitserland) gelden de SNCF-tarieven: 10 € in een transportmand of 20 € aan de lijn met muilkorf.',
    ],
    sncfTip: 'Goed om te weten: SNCF geeft aan dat je ervoor moet zorgen dat je hond andere reizigers niet stoort. Bij problemen kan de conducteur je vragen je dier ergens anders te plaatsen.',

    dbTitle: 'Deutsche Bahn: de hondvriendelijkste trein van Europa',
    dbContent: [
      'Duitsland is waarschijnlijk het land dat honden het meest welkom heet in het openbaar vervoer, en de Deutsche Bahn vormt daarop geen uitzondering.',
      'Een klein huisdier (kattenformaat) in een transportmand reist gratis. Voor een grote hond moet je een kaartje kopen: de halve volwassenenprijs op langeafstandstreinen (ICE, IC, EC), of de volle volwassenenprijs op regionale treinen.',
      'Nieuw: hondenkaartjes zijn nu online verkrijgbaar via de app DB Next Navigator of op next.bahn.de. Geen rijen meer aan het loket!',
      'Lijn en muilkorf zijn verplicht voor grote honden. Ook op het perron moet je hond aan de lijn blijven.',
    ],
    dbTip: 'Het Deutschland-Ticket (maandabonnement) dekt GEEN honden. Je moet een apart hondenkaartje kopen, ook als je dit abonnement hebt.',

    renfeTitle: 'Renfe: uiteenlopende opties in Spanje',
    renfeContent: [
      'Renfe biedt verschillende varianten afhankelijk van het treintype. Op AVE-hogesnelheidstreinen reist je hond voor de meeste trajecten in een bench (max. 60×35×35 cm). Sommige AVE-treinen hebben speciale hondenplaatsen (tot 40 kg) voor 35 € zonder bench.',
      'Op Avlo-treinen (de Spaanse budgetlijn) bedraagt de prijs 10 €. Op Media Distancia-treinen met smalspoor reizen honden gratis aan de lijn met muilkorf.',
      'Let op: je moet je 40 minuten voor vertrek melden bij de Renfe-balie op het station met het vaccinatieboekje en een bewijs van aansprakelijkheidsverzekering. Deze Spaanse eis wordt strikt gecontroleerd.',
    ],

    trenitTitle: 'Trenitalia: Italië heet honden welkom',
    trenitContent: [
      'Trenitalia laat kleine huisdieren (in een transportmand van max. 70×30×50 cm) gratis meereizen op alle treinen. Voor grote honden kost het kaartje 50% van de prijs van de 2e klasse.',
      'Grote honden zijn toegestaan op de Frecciarossa, Frecciargento, Frecciabianca en Intercity, in 1e en 2e klasse. Ze moeten aan de lijn en met muilkorf reizen.',
      'Vergeet niet het certificato di iscrizione all\'anagrafe canina (officiële registratiebewijs) en het libretto sanitario (gezondheidsboekje) van je hond mee te nemen.',
      'Goed nieuws voor de zomer van 2025: tot 15 september 2025 biedt Trenitalia gratis reizen aan voor honden van alle formaten op de Frecce en Intercity.',
    ],

    eurostarTitle: 'Eurostar: één regel die je moet onthouden',
    eurostarContent: [
      'De Eurostar is de maatschappij waarover onze lezers de meeste vragen stellen. De regel is simpel maar absoluut: huisdieren (behalve blindengeleide- en assistentiehonden) zijn VERBODEN op alle treinen door de Kanaaltunnel, dus op alle routes tussen Londen en Parijs, Brussel, Amsterdam en andere Britse bestemmingen.',
      'Op de vastelandroutes van Eurostar (tussen België, Frankrijk, Duitsland en Nederland, zonder via Londen te gaan) zijn dieren daarentegen wel welkom: gratis voor minder dan 6 kg in een transportmand, 30 € voor grote honden aan de lijn met muilkorf.',
      'Wil je met je hond het Kanaal oversteken, dan zijn de enige opties de veerboot (Brittany Ferries, DFDS, P&O) of de Eurotunnel Le Shuttle (voor auto\'s). Honden mogen niet zelfstandig door de Eurotunnel reizen.',
    ],

    tipsTitle: 'Praktische tips voor treinreizen met je hond',
    tips: [
      {
        icon: '📅',
        title: 'Boek van tevoren',
        text: 'Koop het kaartje van je hond tegelijk met dat van jezelf. Vink bij SNCF Connect "Huisdier" aan tijdens het boeken. Gebruik bij DB de app DB Next Navigator.',
      },
      {
        icon: '🎒',
        title: 'Wat je moet meenemen',
        text: 'Vaccinatieboekje, stevige lijn, goed passende muilkorf, water en een opvouwbare bak, snoepjes ter geruststelling, een vertrouwde deken of kussen, poepzakjes.',
      },
      {
        icon: '🏛️',
        title: 'Op het station',
        text: 'Je hond moet op alle perrons aan de lijn blijven. Bij sommige grote stations zijn er groene zones buiten voor een laatste uitlaatbeurt voor vertrek.',
      },
      {
        icon: '🚂',
        title: 'In de trein',
        text: 'Maak het jezelf zo snel mogelijk gemakkelijk. Check of je hond de buren niet stoort. Is hij onrustig, moedig hem dan aan om onder de stoel te gaan liggen. Informeer de conducteur over de aanwezigheid van je dier.',
      },
      {
        icon: '😰',
        title: 'Als je hond niet tot rust komt',
        text: 'Bereid je hond voor op de reis: laat hem wennen aan de muilkorf, treingeluiden en kleine ruimtes. Een korte oefenreis kan helpen. Raadpleeg bij ernstige angst je dierenarts (natuurlijke of voorgeschreven kalmerende middelen).',
      },
      {
        icon: '🚫',
        title: 'Bij problemen onderweg',
        text: 'Klaagt een andere reiziger of is iemand allergisch, dan kan de conducteur je vragen je hond te verplaatsen. Wordt de toegang in zeldzame gevallen geweigerd, vraag dan een onregelmatigheidsverslag aan voor een terugbetaling.',
      },
    ],

    interrailTitle: 'Reizen met een Interrail-pas en een hond',
    interrailText: 'Je Interrail-pas dekt je eigen reis, maar dierenkaartjes moet je apart kopen bij elke spoorwegmaatschappij. De lokale regels van elk land zijn van toepassing. Let op: de Eurostar (Kanaaltunnel) blijft verboden voor dieren, ook met een Interrail-pas.',

    faqTitle: 'Veelgestelde vragen',
    faqs: [
      {
        q: 'Heeft mijn hond een kaartje nodig voor de trein?',
        a: 'Dat hangt af van zijn formaat. In de meeste Europese landen reizen kleine huisdieren in een geschikte transportmand gratis. Grote honden hebben een betaald kaartje nodig. In Frankrijk (SNCF) reken je op 10 € (transportmand) of 20 € (aan de lijn). In Duitsland (DB) is het de halve volwassenenprijs op langeafstandstreinen.',
      },
      {
        q: 'Heeft mijn hond een muilkorf nodig in de trein?',
        a: 'Ja, in de meeste landen en bij de meeste maatschappijen. In Frankrijk (SNCF, Ouigo) is de muilkorf verplicht voor honden boven 6 kg. In Italië (Trenitalia), Oostenrijk (ÖBB) en Nederland (NS) is dit ook verplicht voor grote honden. In België (SNCB) wordt het aanbevolen, maar niet altijd streng gecontroleerd.',
      },
      {
        q: 'Kan ik mijn hond in het bagageruim van een trein zetten?',
        a: 'Nee. In tegenstelling tot vliegtuigen hebben Europese treinen geen laadruim voor levende dieren. Je hond moet altijd bij je zijn: hetzij in een transportmand in het reizigersgedeelte, hetzij aan de lijn aan je voeten.',
      },
      {
        q: 'Kan ik met een hond reizen op de Eurostar?',
        a: 'Nee, niet op treinen door de Kanaaltunnel (van/naar Londen). Op die routes zijn dieren om hygiënische redenen strikt verboden. Alleen geregistreerde assistentiehonden mogen meereizen. Om met een hond het Kanaal over te steken, neem je de veerboot of de Eurotunnel Le Shuttle (alleen voor auto\'s).',
      },
      {
        q: 'Mag mijn hond van een "gevaarlijk" ras met de trein?',
        a: 'Honden van categorie 1 (pitbull-type, American Staffordshire Terrier, Japanse Mastiff, Tosa) zijn verboden op SNCF- en Ouigo-treinen. Honden van categorie 2 (Rottweiler, Staffordshire Bull Terrier, Dobermann, enz.) mogen wel mee, maar moeten altijd een muilkorf dragen en aan de lijn blijven. Informeer bij elke maatschappij naar de regels voor andere landen.',
      },
      {
        q: 'Moet mijn hond de hele reis op zijn plek blijven?',
        a: 'Ja, zo veel mogelijk. Bij SNCF moet je hond aan je voeten reizen, aan de lijn en met muilkorf, en mag hij geen passagiersstoel innemen. In sommige regionale TER-treinen kan hij in het gangpad of het instapgedeelte plaatsnemen.',
      },
      {
        q: 'Moet ik de vaccinaties van mijn hond in de trein laten zien?',
        a: 'In Frankrijk niet standaard verplicht, maar het wordt sterk aanbevolen om het gezondheidsboekje bij je te hebben. In Spanje (Renfe) is het tonen van het vaccinatieboekje bij de check-in verplicht. In Italië (Trenitalia) wordt het gezondheidsboekje geëist. Reis altijd met de documenten van je dier.',
      },
    ],

    sourcesTitle: 'Geverifieerde bronnen',
    sourcesText: 'Deze gids is gebaseerd op officiële informatie van SNCF, Ouigo, Deutsche Bahn, Renfe, Trenitalia, Eurostar, SNCB, NS, SBB, ÖBB en PKP Intercity, geverifieerd in 2025. Prijzen en regels kunnen veranderen, controleer altijd de website van de maatschappij voor je vertrekt.',

    internalLinksTitle: 'Gerelateerde gidsen',
    guidePasseport: 'Europees huisdierenpaspoort: complete gids',
    guideAll: 'Al onze praktische gidsen',
  },
  it: {
    hero: 'Viaggiare in treno con il cane in Europa',
    subtitle: 'SNCF, Deutsche Bahn, Renfe, Trenitalia, Eurostar... La guida completa su regole, tariffe, obbligo di museruola e consigli pratici per viaggiare in treno con il tuo cane senza stress.',
    lastUpdate: 'Aggiornato a',
    breadcrumbGuides: 'Guide',
    breadcrumbCurrent: 'Treno con il cane',
    badge: 'Guida pratica',

    keyFactsTitle: 'Le regole essenziali da ricordare',
    keyFacts: [
      { icon: '🎫', title: 'Biglietto obbligatorio', text: 'In tutta Europa, un cane di grande taglia ha bisogno di un biglietto. Gli animali piccoli in trasportino viaggiano spesso gratis.' },
      { icon: '😷', title: 'Museruola richiesta', text: 'In Francia (SNCF, Ouigo), Italia, Austria e nella maggior parte dei paesi, la museruola è obbligatoria per i cani oltre i 6 kg.' },
      { icon: '🚫', title: 'Tunnel Eurostar = vietato', text: 'Gli animali domestici (tranne i cani guida) sono severamente vietati sui treni Eurostar che attraversano il tunnel della Manica tra Londra e l\'Europa continentale.' },
      { icon: '📋', title: 'Libretto sanitario', text: 'Porta sempre con te il libretto di vaccinazione del tuo cane. Renfe (Spagna) richiede anche un\'assicurazione di responsabilità civile.' },
      { icon: '📏', title: 'Misure del trasportino', text: 'Il formato standard per gli animali piccoli è 45×30×25 cm (SNCF). Alcuni operatori accettano fino a 70×30×50 cm.' },
      { icon: '🐕‍🦺', title: 'Cani guida gratis', text: 'I cani guida e da assistenza viaggiano gratis in tutta Europa, senza obbligo di museruola.' },
    ],

    tableTitle: 'Confronto tra le compagnie ferroviarie europee',
    colOperator: 'Compagnia',
    colAllowed: 'Ammesso?',
    colTicket: 'Biglietto?',
    colPrice: 'Tariffa',
    colSize: 'Formato',
    colMuzzle: 'Museruola',
    colCarrier: 'Trasportino',
    colMax: 'Max / passeggero',
    colNotes: 'Note',

    allowedYes: 'Sì',
    allowedPartial: 'Parziale',
    allowedNo: 'No',

    sncfTitle: 'SNCF e Ouigo: viaggiare con il cane in Francia',
    sncfContent: [
      'SNCF è la compagnia più importante per i nostri lettori. Le regole sono chiare e piuttosto permissive.',
      'Sui TGV INOUI e Intercités hai due opzioni: (1) il tuo cane pesa meno di 6 kg e viaggia in un trasportino chiuso (max 45×30×25 cm) per 10 €; (2) il tuo cane è più grande e viaggia al guinzaglio, con museruola, ai tuoi piedi per 20 €.',
      'Con OUIGO, il marchio low-cost di SNCF, è ancora più semplice: una tariffa unica di 10 € per tutti i cani, indipendentemente dalla taglia. La museruola resta obbligatoria.',
      'I cani di categoria 1 (tipo pit-bull, mastiff, tosa) sono severamente vietati su tutti i treni. I cani di categoria 2 possono viaggiare, ma devono indossare la museruola e stare al guinzaglio.',
      'Prenota il biglietto del tuo cane insieme al tuo su SNCF Connect o sull\'app OUIGO. Un solo animale per passeggero adulto.',
      'Sui TGV Lyria (Francia–Svizzera) si applicano le tariffe SNCF: 10 € in trasportino o 20 € al guinzaglio con museruola.',
    ],
    sncfTip: 'Buono a sapersi: SNCF precisa che devi assicurarti che il tuo cane non disturbi gli altri passeggeri. In caso di problemi, il controllore può chiederti di sistemare diversamente il tuo animale.',

    dbTitle: 'Deutsche Bahn: il treno più dog-friendly d\'Europa',
    dbContent: [
      'La Germania è probabilmente il paese più accogliente per i cani nei trasporti pubblici, e la Deutsche Bahn non fa eccezione.',
      'Un animale piccolo (taglia gatto) in un trasportino viaggia gratis. Per un cane grande devi acquistare un biglietto: metà tariffa adulto sulle grandi linee (ICE, IC, EC), oppure tariffa adulto intera sui treni regionali.',
      'Novità importante: i biglietti per cani sono ora disponibili online tramite l\'app DB Next Navigator o su next.bahn.de. Niente più code in stazione!',
      'Guinzaglio e museruola sono obbligatori per i cani grandi. In stazione, il tuo cane deve stare al guinzaglio anche sui binari.',
    ],
    dbTip: 'Il Deutschland-Ticket (abbonamento mensile) NON copre i cani. Devi acquistare un biglietto animale separato anche se possiedi questo abbonamento.',

    renfeTitle: 'Renfe: opzioni diverse in Spagna',
    renfeContent: [
      'Renfe propone diverse formule a seconda del tipo di treno. Sui treni AVE (alta velocità), il tuo cane viaggia in una gabbia (max 60×35×35 cm) per la maggior parte dei tragitti. Alcuni treni AVE dispongono di posti speciali per cani (fino a 40 kg) a 35 € senza gabbia.',
      'Sui treni Avlo (il low-cost spagnolo), la tariffa è di 10 €. Sui treni Media Distancia a scartamento metrico, i cani viaggiano gratis al guinzaglio con museruola.',
      'Attenzione: devi presentarti al banco Renfe in stazione 40 minuti prima della partenza con il libretto di vaccinazione e un attestato di assicurazione di responsabilità civile. È una specificità spagnola da non trascurare.',
    ],

    trenitTitle: 'Trenitalia: l\'Italia accoglie i cani',
    trenitContent: [
      'Trenitalia permette agli animali piccoli (in un trasportino max 70×30×50 cm) di viaggiare gratis su tutti i suoi treni. Per i cani grandi, il biglietto viene venduto al 50% della tariffa di 2ª classe.',
      'I cani grandi sono ammessi su Frecciarossa, Frecciargento, Frecciabianca e Intercity, in 1ª e 2ª classe. Devono stare al guinzaglio e indossare la museruola.',
      'Non dimenticare di portare il certificato di iscrizione all\'anagrafe canina e il libretto sanitario del tuo cane.',
      'Buona notizia per l\'estate 2025: fino al 15 settembre 2025, Trenitalia offre la gratuità ai cani di tutte le taglie su Frecce e Intercity.',
    ],

    eurostarTitle: 'Eurostar: una regola da tenere bene a mente',
    eurostarContent: [
      'L\'Eurostar è il treno che genera più domande da parte dei nostri lettori. La regola è semplice ma assoluta: gli animali domestici (tranne i cani guida e da assistenza) sono VIETATI su tutti i treni che attraversano il tunnel della Manica, cioè quelli che collegano Londra a Parigi, Bruxelles, Amsterdam e altre destinazioni britanniche.',
      'Sulle tratte continentali dell\'Eurostar (tra Belgio, Francia, Germania e Paesi Bassi, senza passare per Londra), invece, gli animali sono i benvenuti: gratis per i cani sotto i 6 kg in trasportino, 30 € per i cani grandi al guinzaglio con museruola.',
      'Se vuoi attraversare la Manica con il tuo cane, le uniche opzioni sono il traghetto (Brittany Ferries, DFDS, P&O) o il treno Eurotunnel Le Shuttle (per le auto). I cani non possono viaggiare da soli nell\'Eurotunnel.',
    ],

    tipsTitle: 'Consigli pratici per viaggiare in treno con il cane',
    tips: [
      {
        icon: '📅',
        title: 'Prenota in anticipo',
        text: 'Acquista il biglietto del tuo cane insieme al tuo. Su SNCF Connect, spunta "Animale domestico" durante la prenotazione. Su DB, usa l\'app DB Next Navigator.',
      },
      {
        icon: '🎒',
        title: 'Cosa portare',
        text: 'Libretto di vaccinazione, guinzaglio robusto, museruola adatta, acqua e una ciotola pieghevole, snack per rassicurare, una coperta o cuscino familiare, sacchetti igienici.',
      },
      {
        icon: '🏛️',
        title: 'In stazione',
        text: 'Il tuo cane deve stare al guinzaglio su tutti i binari. In alcune stazioni, ci sono spazi verdi all\'esterno per un\'ultima uscita prima della partenza.',
      },
      {
        icon: '🚂',
        title: 'In treno',
        text: 'Sistemati il prima possibile. Controlla che il tuo cane non disturbi i vicini. Se è agitato, incoraggialo a sdraiarsi sotto il sedile. Non esitare a informare il controllore della presenza del tuo animale.',
      },
      {
        icon: '😰',
        title: 'Se il tuo cane rifiuta di calmarsi',
        text: 'Prepara il tuo cane prima del viaggio: abituazione alla museruola, ai rumori del treno, agli spazi ristretti. Un viaggio di allenamento su un tragitto breve può aiutare. In caso di forte ansia, consulta il tuo veterinario (ansiolitici naturali o su prescrizione).',
      },
      {
        icon: '🚫',
        title: 'In caso di problemi a bordo',
        text: 'Se un altro passeggero si lamenta o è allergico, il controllore può chiederti di spostare il tuo cane. Nei rari casi di rifiuto di imbarco, richiedi un rapporto di irregolarità per ottenere un rimborso.',
      },
    ],

    interrailTitle: 'Viaggiare con un Interrail pass e un cane',
    interrailText: 'L\'Interrail pass copre il tuo biglietto di viaggiatore, ma i biglietti animale devono essere acquistati separatamente presso ogni compagnia ferroviaria. Si applicano le regole locali di ogni paese. Nota che l\'Eurostar (tunnel) resta vietato agli animali anche con un Interrail pass.',

    faqTitle: 'Domande frequenti',
    faqs: [
      {
        q: 'Il mio cane ha bisogno di un biglietto per il treno?',
        a: 'Dipende dalla taglia. Nella maggior parte dei paesi europei, gli animali piccoli in un trasportino adatto viaggiano gratis. I cani grandi richiedono un biglietto a pagamento. In Francia (SNCF), calcola 10 € (trasportino) o 20 € (al guinzaglio). In Germania (DB), è la metà della tariffa adulto sulle grandi linee.',
      },
      {
        q: 'Serve la museruola per il mio cane in treno?',
        a: 'Sì, nella maggior parte dei paesi e delle compagnie. In Francia (SNCF, Ouigo), la museruola è obbligatoria per i cani oltre i 6 kg. In Italia (Trenitalia), Austria (ÖBB) e Paesi Bassi (NS), è obbligatoria anche per i cani grandi. In Belgio (SNCB), è consigliata ma non sempre imposta rigorosamente.',
      },
      {
        q: 'Posso mettere il mio cane nel bagagliaio di un treno?',
        a: 'No. A differenza degli aerei, non esiste un vano bagagli per animali vivi nei treni europei. Il tuo cane deve sempre stare con te: in un trasportino nella zona passeggeri, oppure al guinzaglio ai tuoi piedi.',
      },
      {
        q: 'Posso viaggiare con un cane sull\'Eurostar?',
        a: 'No, non sui treni che attraversano il tunnel della Manica (da/per Londra). Gli animali sono severamente vietati su queste tratte per motivi sanitari. Solo i cani guida registrati possono viaggiare. Per attraversare la Manica con un cane, bisogna prendere il traghetto o il treno Eurotunnel Le Shuttle (solo auto).',
      },
      {
        q: 'Il mio cane di razza "pericolosa" può prendere il treno?',
        a: 'I cani di categoria 1 (tipo pit-bull, American Staffordshire Terrier, Mastino Giapponese, Tosa) sono vietati sui treni SNCF e Ouigo. I cani di categoria 2 (Rottweiler, Staffordshire Bull Terrier, Dobermann, ecc.) possono viaggiare ma devono obbligatoriamente indossare la museruola e stare al guinzaglio. Informati presso ogni compagnia per gli altri paesi.',
      },
      {
        q: 'Il mio cane deve restare al suo posto per tutto il viaggio?',
        a: 'Sì, per quanto possibile. Su SNCF, il tuo cane deve viaggiare ai tuoi piedi, al guinzaglio e con museruola. Non può occupare un posto passeggero. Su alcuni TER regionali, può trovarsi nello spazio tra i sedili o nel corridoio se sei in una carrozza di testa.',
      },
      {
        q: 'Devo presentare i vaccini del mio cane in treno?',
        a: 'Non sempre obbligatorio in Francia, ma è fortemente consigliato avere con sé il libretto sanitario. In Spagna (Renfe), la presentazione del libretto di vaccinazione è obbligatoria al check-in. In Italia (Trenitalia), è richiesto il libretto sanitario. Viaggia sempre con i documenti del tuo animale.',
      },
    ],

    sourcesTitle: 'Fonti verificate',
    sourcesText: 'Questa guida si basa su informazioni ufficiali di SNCF, Ouigo, Deutsche Bahn, Renfe, Trenitalia, Eurostar, SNCB, NS, SBB, ÖBB e PKP Intercity, verificate nel 2025. Tariffe e regole possono cambiare, consulta sempre il sito della compagnia prima del tuo viaggio.',

    internalLinksTitle: 'Guide correlate',
    guidePasseport: 'Passaporto europeo per animali: guida completa',
    guideAll: 'Tutte le nostre guide pratiche',
  },
}
